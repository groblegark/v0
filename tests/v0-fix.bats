...
@test "v0-fix: --after accepts operation name and resolves to epic_id" {
    # Create mock wk
    cat > "${TEST_TEMP_DIR}/mock-v0-bin/wk" <<'EOF'
#!/bin/bash
echo "wk $*" >> "$MOCK_CALLS_DIR/wk.calls" 2>/dev/null || true
...
EOF
    ...
    # Run v0 fix with --after using operation name
    run "${V0_FIX}" --after blocker-op "Test bug" 2>&1 || true
...
@test "v0-fix: --after accepts mix of operation names and wok IDs" {
    cat > "${TEST_TEMP_DIR}/mock-v0-bin/wk" <<'EOF'
#!/bin/bash
echo "wk $*" >> "$MOCK_CALLS_DIR/wk.calls" 2>/dev/null || true
if [[ "$1" == "new" ]]; then
...
EOF
    ...
    run "${V0_FIX}" --after my-op,test-direct456 "Mixed dependencies" 2>&1 || true
...
@test "v0-fix: --after warns on unresolvable operation name" {
    cat > "${TEST_TEMP_DIR}/mock-v0-bin/wk" <<'EOF'
#!/bin/bash
if [[ "$1" == "new" ]]; then
    echo "test-999"
...
EOF
    ...
    # No operation exists
    run "${V0_FIX}" --after nonexistent-op "Bug with bad blocker" 2>&1 || true
...
@test "v0-fix: --after skips operations without epic_id" {
    cat > "${TEST_TEMP_DIR}/mock-v0-bin/wk" <<'EOF'
#!/bin/bash
echo "wk $*" >> "$MOCK_CALLS_DIR/wk.calls" 2>/dev/null || true
if [[ "$1" == "new" ]]; then
...
EOF
    ...
    run "${V0_FIX}" --after early-op "Bug blocked by early op" 2>&1 || true
